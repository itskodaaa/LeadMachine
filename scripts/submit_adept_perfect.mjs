...
(async () => {
  ...
  page.on('response', async res => {
    if (res.url().includes('admin-ajax.php')) {
      console.log('WPForms AJAX Response:', res.status(), res.url());
      try {
  ...
  await page.keyboard.type('Pamela', { delay: 50 });

  await page.focus('#wpforms-78286-field_1-last');
  await page.keyboard.type('Jameson', { delay: 50 });

  await page.focus('#wpforms-78286-field_5');
  await page.keyboard.type('Northeast Precision Machinery, Inc.', { delay: 30 });

  await page.focus('#wpforms-78286-field_6');
  await page.keyboard.type('708-568-3708', { delay: 50 });

  await page.focus('#wpforms-78286-field_2');
  await page.keyboard.type('pamela.jameson@nortiheastprecision.com', { delay: 30 });

  console.log('Waiting 12 seconds to satisfy WPForms anti-bot time delay...');
  ...
  if (confirmation && (confirmation.toLowerCase().includes('thank') || confirmation.toLowerCase().includes('contact') || confirmation.toLowerCase().includes('received'))) {
