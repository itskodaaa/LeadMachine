...
async function run() {
  ...
  console.log('Navigating to https://atdpmfg.com/contact/ ...');
  await page.goto('https://atdpmfg.com/contact/', { waitUntil: 'networkidle2', timeout: 30000 });
  ...
  await page.evaluate((p) => {
    ...
    // Email
    setVal('input_1_5', p.email);
    // Message
    setVal('input_1_4', p.message);
  }, OUTREACH_PROFILE);
  ...
  if (result.confText || result.bodySnippet.toLowerCase().includes('thanks for contacting') || result.bodySnippet.toLowerCase().includes('received your message') || result.bodySnippet.toLowerCase().includes('will be in touch')) {
    ...
    const current = db.prepare('SELECT notes FROM leads WHERE id = 1332').get();
    const note = `Contact form: https://atdpmfg.com/contact/ (Autofilled & verified: ${confirmationMsg.slice(0, 120)})`;
    const newNotes = current?.notes ? current.notes + ' | ' + note : note;

    db.transaction(() => {
      updateStmt.run(newNotes, 'contacted', 1332);
      logStmt.run(1332, 'sent', note);
    })();
    console.log('Updated lead #1332 to contacted in DB!');
  } else {
