...
async function run() {
  ...
  try {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      console.log(`Headed Poll ${i}:`, JSON.stringify(res));
    ...
    if (confirmed) {
      console.log(`✅ Lead #4592 SUBMISSION CONFIRMED in headed mode: "${confirmText}"`);
      ...
      console.log('❌ Submission not confirmed in headed mode (blocked by GoDaddy invisible reCAPTCHA)');
      const note = 'Contact form: https://twistedmetalswelding.com (Blocked by GoDaddy invisible Google reCAPTCHA)';
      db.prepare('UPDATE leads SET status = ?, notes = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run('unable_to_reach', note, 4592);
      db.prepare('INSERT INTO contact_logs (lead_id, action, notes, created_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)').run(4592, 'bounced', note);
    }
  } catch (e) {
